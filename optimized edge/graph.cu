...
#include <string>
#define WARP_SIZE 32
__global__ void filter_kernel(unsigned int* input, unsigned int* output, unsigned int N,
    unsigned int* M) {
...
void truss_gpu(Graph * g, int k)
{
    Graph graph_d;
    int *done_d, *count_d;
    unsigned int *filterOutput, *filterM;

    Timer timer;
    startTime(&timer);
    cudaMalloc((void**)&graph_d.row, g->nnzSize*sizeof(int));
    cudaMalloc((void**)&graph_d.col, g->nnzSize*sizeof(int));
    cudaMalloc((void**)&graph_d.index, g->nnzSize*sizeof(int));
    cudaMalloc((void**)&filterOutput, g->nnzSize*sizeof(int));
    ...
    cudaMalloc((void **)&filterM, sizeof(int));
    ...
    cudaMemcpy(graph_d.index, g->index, g->nnzSize*sizeof(int), cudaMemcpyHostToDevice);
    ...
    *count = 0;
    ...
    while (!*done)
    {
        int * filterCounter = (int *) malloc(sizeof(int));
        *filterCounter = 0;
        cudaMemcpy(filterM, filterCounter, sizeof(int), cudaMemcpyHostToDevice);
        unsigned int numThreads = 1024;
        graph_d.nnzSize -= *count;
        unsigned int numBlocks = ((graph_d.nnzSize) + numThreads - 1) / numThreads;
        *done = 1;
        ...
        cudaDeviceSynchronize();
        filter_kernel<<<numThreads, numBlocks>>>(graph_d.index, filterOutput, graph_d.nnzSize, filterM);
        unsigned int * temp = filterOutput;
        filterOutput = graph_d.index;
        graph_d.index = temp;
        cudaMemcpy(count, filterM, sizeof(int), cudaMemcpyDeviceToHost);
        cudaDeviceSynchronize();
    }
    stopTime(&timer);
