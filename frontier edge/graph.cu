...
#include "../timer.h"
#include <string>
__global__ void truss_kernel(Graph g, int k, int *done, int iteration)
{
    unsigned int index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index < g.nnzSize) {
        unsigned int v1 = g.row[index];
        unsigned int v2 = g.col[index];
        if (v2 != UINT_MAX/* && (g.affected[index] || iteration == 0)*/) {
            ...
            g.affected[index] = g.affected[index] || affected;
            for (unsigned int n1 = g.rowPtr[v1]; n1 < g.rowPtr[v1+1]; ++n1) {
                g.affected[n1] = g.affected[index] || affected;
            }
            for (unsigned int n1 = g.rowPtr[v2]; n1 < g.rowPtr[v2+1]; ++n1) {
                g.affected[n1] = g.affected[index] || affected;
            }
        }
    }
...
void truss_gpu(Graph * g, int k)
{
    ...
    cudaMalloc((void**)&graph_d.affected, g->nnzSize*sizeof(int));
    ...
    cudaMemcpy(graph_d.affected, g->affected, g->nnzSize*sizeof(int), cudaMemcpyHostToDevice);
    ...
    *done = 0;
    ...
    while (!*done)
    {
        *done = 1;
        ...
        truss_kernel<<<numThreads, numBlocks>>>(graph_d, k, done_d, iteration++);
